             ...
             <AccordionTrigger className="p-4 hover:no-underline">
                <div className="flex justify-between items-center w-full">
                    <div className="flex flex-col text-left">
                        <span className="font-bold capitalize text-base text-foreground">
                ...
                <div className="space-y-4">
                    {workout.notes && (
                        <div className="p-3 app-card-content">
                           <h4 className="font-semibold text-sm mb-2 flex items-center gap-2 text-foreground"><StickyNote className="h-4 w-4 text-accent-yellow" /> Notes de séance</h4>
                           <p className="text-sm text-muted-foreground whitespace-pre-wrap">{workout.notes}</p>
                        </div>
                    )}

                    <div>
                        <h4 className="font-semibold text-sm mb-2 flex items-center gap-2 text-foreground"><List className="h-4 w-4 text-primary" /> Exercices</h4>
                        <ul className="space-y-3">
                            {workout.exercises.map(exercise => (
                                <li key={exercise.id} className="text-sm app-card-content p-3">
                                    <div className="flex items-start justify-between gap-2 mb-2">
                                        <p className="font-semibold text-base break-words flex-1 text-foreground">{exercise.name}</p>
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                    ...
                                    <ul className="space-y-1 pl-2 border-l-2 border-border">
                                        {exercise.sets.map((set) => (
                                            <li key={set.id} className="text-muted-foreground">
                                                - <span className="font-semibold text-foreground">{set.weight} kg</span> x <span className="font-semibold text-foreground">{set.reps} reps</span>
                                            </li>
                                        ))}
                                    </ul>
                                    {exercise.notes && (
                                         <div className="mt-2 pt-2 border-t border-border/50">
                                            <p className="text-xs text-muted-foreground whitespace-pre-wrap italic">Note: {exercise.notes}</p>
                                         </div>
                                    )}
                                </li>
                            ))}
