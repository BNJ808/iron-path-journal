...
export const WorkoutPlanCard = ({ plan, onUpdate, onDelete }: WorkoutPlanCardProps) => {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        `${plan.color} text-primary-foreground rounded-2xl shadow-md hover:shadow-lg transition-all duration-200 cursor-grab active:cursor-grabbing`,
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            size="sm"
            className="h-6 w-6 p-0 hover:bg-primary-foreground/20 flex-shrink-0 text-primary-foreground"
            onClick={(e) => {
              e.stopPropagation();
              e.preventDefault();
            }}
