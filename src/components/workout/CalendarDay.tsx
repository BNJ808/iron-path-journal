...
export const CalendarDay = ({
  ...
  const calendarDayContent = (
    <div
      ref={setNodeRef}
      className={cn(
        "border rounded-lg transition-all duration-200 relative bg-card",
        ...
        {scheduledPlans.map(planId => {
          ...
          return (
            <div
              key={planId}
              className={cn(
                `${plan.color} text-primary-foreground rounded shadow-sm transition-all hover:shadow-md group relative`,
              ...
              style={{
                touchAction: isDeleteMode ? 'manipulation' : 'none',
              }}
            >
              <div className={cn(
